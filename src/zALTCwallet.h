...
#ifndef ALTC_ZALTCWALLET_H
#define ALTC_ZALTCWALLET_H
...
public:
    CzALTCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZALTC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZALTC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //ALTC_ZALTCWALLET_H

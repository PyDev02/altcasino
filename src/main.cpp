...
#if defined(NDEBUG)
#error "ALTC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uALTC) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
   ...
   if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
       return error("%s : zALTC spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zALTC is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zALTC spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zALTC without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zALTC spend with serial %s from tx %s is not in valid range\n", __func__,
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    if (GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin transactions are not supported by ALTC"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zALTC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zALTC");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zALTC mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    else if (nHeight == 1) {nSubsidy = 639000 * COIN;} //Premine
    else if (nHeight < 100) {nSubsidy = 30 * COIN;} //PoW Phase
    else if (nHeight < 1000) {nSubsidy = 31 * COIN;} //PoW Phase
    else if (nHeight < 10000) {nSubsidy = 35 * COIN;} //Phase 1 (20 blocks where part of PoW)
    else if (nHeight < 30000) {nSubsidy = 20 * COIN;} //Phase 2
    else if (nHeight < 50000) {nSubsidy = 18 * COIN;} //Phase 3
    else if (nHeight < 70000) {nSubsidy = 16 * COIN;} //Phase 4
    else if (nHeight < 90000) {nSubsidy = 14 * COIN;} //Phase 5
    else if (nHeight < 100000) {nSubsidy = 12 * COIN;} //Phase 6
    else if (nHeight < 130000) {nSubsidy = 10 * COIN;} //Phase 7
    else if (nHeight < 150000) {nSubsidy = 8 * COIN;} //Phase 8
    else if (nHeight > 170000) {nSubsidy = 5 * COIN;} //Phase 9
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZALTCStake)
    ...
    if (nHeight < 100) {ret = blockValue * 0;} 
    else if (nHeight < 210) {ret = blockValue * .2;}
	else if (nHeight >= 210) {ret = blockValue * 0.8;} // 80% of block paid to nodes
    return ret;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from ALTC
...
{
    RenameThread("ALTC-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZALTCMinted()
{
...
void RecalculateZALTCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zALTC supply
...
bool RecalculateALTCSupply(int nHeightStart)
{
...
{
    // ALTC: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZALTCSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
        ...
        if (block.nTime > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE) && !IsInitialBlockDownload() && tx.ContainsZerocoins()) {
            return state.DoS(100, error("ConnectBlock() : zerocoin transactions are not supported by ALTC"));
        }
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculateZALTCMinted();
        RecalculateZALTCSpent();
        RecalculateALTCSupply(Params().Zerocoin_StartHeight());
    }

    //Track zALTC money supply in the block index
    if (!UpdateZALTCSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zALTC supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ALTC to zALTC
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zALTC spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zALTC serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    ...
    if (CZSpdStake* zALTC = dynamic_cast<CZSpdStake*>(stake)) {
        ...
        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
             return error("%s: zALTC stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZALTC() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zALTC fails context checks", __func__));
    ...
    if (isPoS) {
       ...
       // Inputs
       std::vector<CTxIn> ALTCInputs;
       std::vector<CTxIn> zALTCInputs;

       for (const CTxIn& stakeIn : stakeTxIn.vin) {
           if(stakeIn.scriptSig.IsZerocoinSpend()){
               zALTCInputs.push_back(stakeIn);
           }else{
               ALTCInputs.push_back(stakeIn);
           }
       }
       const bool hasALTCInputs = !ALTCInputs.empty();
       const bool hasZALTCInputs = !zALTCInputs.empty();
       ...
       for (const CTransaction& tx : block.vtx) {
           for (const CTxIn& in: tx.vin) {
               ...
               if(hasALTCInputs)
                   ...
                   for (const CTxIn& ALTCIn : ALTCInputs){
                       if(ALTCIn.prevout == in.prevout){
       ...
       if (isBlockFromFork) {
           ...
           do {
               ...
               for (const CTransaction& t : bl.vtx) {
                   for (const CTxIn& in: t.vin) {
                       ...
                       for (const CTxIn& stakeIn : ALTCInputs) {
           ...
           // Now that this loop if completed. Check if we have zALTC inputs.
           if(hasZALTCInputs){
               for (const CTxIn& zPivInput : zALTCInputs) {
                   ...
                   if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                       return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                  stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zALTC");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zALTC mints and %d zALTC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ALTC: We use certain sporks during IBD, so check to see if they are
